module.exports = (models) => {
  ...
  return {
    ...
    postCorpsEnumeration: async function(request, h) {
      try {
        ...
        for (let qI = 0; qI < queues; qI ++) {
          ...
          processQSync({search: search.id, queue: qI, start: enumStart, end: enumEnd});
          enumStart = enumEnd + 1;
        }
    ...
    getSearchEvents: (request, h) => {
      let { wss, ws } = request.websocket();
      let _ws = wss;
      let selfHost = process.env.SELF_HOST;
      if (selfHost.indexOf('localhost') === 0) {
        _ws = ws;
      }
      _ws.send(JSON.stringify({ cmd: "HELLO", arg: "OPEN"}));
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        for (let i = params.start; i <= params.end; i ++) {
          ...
          searchEmitter.emit('event', {search: params.search, start: params.start, end: params.end, queue: params.queue, status: 'in-progress', id: id});
        }
        searchEmitter.emit('event', {search: params.search, start: params.start, end: params.end, queue: params.queue, status: 'complete', id: id});
      } catch (e) {
        searchEmitter.emit('event', {search: params.search, start: params.start, end: params.end, queue: params.queue, status: 'error', id: id});
