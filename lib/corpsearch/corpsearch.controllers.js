module.exports = (models) => {
    ...
    try {
      ...
      do {
        if (pageTitle == "Runtime Error") {
          console.log("Runtime Error, Initial Page Navigate; try ", tries, "; entity ", entityId);
        ...
        tries ++;
        // if our tries are 6 and we still have Runtime Error, it means we're
        // still getting a 500 but we aren't going to try again. log it out
        if (tries == 6 && pageTitle == "Runtime Error") {
          console.log("Runtime Error five times in a row. Giving Up.");
          throw "Runtime Error five times in a row. Giving Up.";
        }
      } while (tries <= 5 && pageTitle == "Runtime Error");
  ...
  async function getDBCorps(query = {}) {
    ...
    if (query.address) {
      whereOptions['details.Address'] = {[Op.iLike]: `%${query.address}%`};
    }
    // NOTE: When searching in the officers JSONB array, we just cast the field
    // to text and query the whole thing.
    // officer entry
    if (query.officers) {
      whereOptions[Op.and] = sequelize.where(
        sequelize.cast(sequelize.col('officers'), 'text'),
        {
          [Op.iLike]: `%${query.officers}%`,
        }
      );
    }
    // We build up filters for later so we can ensure the text we are looking for is in the key/value we are looking for.
    let officerFilters = [];
    if (query.officersName) {
      whereOptions[Op.and] = sequelize.where(
        sequelize.cast(sequelize.col('officers'), 'text'),
        {
          [Op.iLike]: `%${query.officersName}%`,
        }
      );
      officerFilters.push([query.officersName, 'Name']);
    }
    if (query.officersTitle) {
      whereOptions[Op.and] = sequelize.where(
        sequelize.cast(sequelize.col('officers'), 'text'),
        {
          [Op.iLike]: `%${query.officersTitle}%`,
        }
      );
      officerFilters.push([query.officersTitle, 'Title']);
    }
    if (query.officersAddress) {
      whereOptions[Op.and] = sequelize.where(
        sequelize.cast(sequelize.col('officers'), 'text'),
        {
          [Op.iLike]: `%${query.officersAddress}%`,
        }
      );
      officerFilters.push([query.officersAddress, 'Address']);
    }
    ...
    console.log(options)
    let results = await models.Corpsearch.findAll(options);

    // Go through built up filters and test them all at once so we only have to
    // iterate the results set once
    if (officerFilters.length > 0) {
      results = results.filter((company) => {
        // .find will return true if it finds at least one occurance of an officer matching all filter criteria
        return company.officers.find((officer) => {
          // Evaluate each officer against all filter criteria. If it doesn't
          // pass any of the tests, then it fails filter criteria. That company
          // can still make it through the filter though if another officer
          // matches all the criteria in this .find
          for (var filter of officerFilters) {
            // check if our query value is in the key/value we are looking for it in
            if (!RegExp(`${filter[0]}`, 'i').test(officer[filter[1]])) {
              return false;
            }
          }
          return true;
